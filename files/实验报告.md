...
one-hot编码又称为独热编码、一位有效编码。其方法是使用呎位状态寄存器来对N个状态进行编码，每个状态都有它独立的寄存器位，并且在任意时候，其中只有一位有效。采用呯呮呥吭周呯呴编码能够较好地反映数据的特征，并对后续的embedding的处理有较好的结果。但是，如果只是采用呯呮呥吭周呯呴编码的话，我们可以明显发现，这样的编码结果使得数据量急剧变大，会导致我们无法对其进行处理。比如，以本次实验数据集为例，我们的user_id和item_id的数量分别达到了19835和602305个，如果采用one-hot编码的话，数据规模将会非常庞大，导致数据处理和模型训练变得非常困难。
